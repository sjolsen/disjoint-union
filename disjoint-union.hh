#ifndef DISJOINT_UNION_HH
#define DISJOINT_UNION_HH
...
public:
	...
	create ()
	{
		...
		new (&result._storage) nth_type <index, T...> ();
		return result;
	}

	template <std::size_t index>
	static std::enable_if_t <index < sizeof... (T), disjoint_union>
	create (const nth_type <index, T...>& obj)
	{
		disjoint_union result (index);
		new (&result._storage) nth_type <index, T...> (obj);
		return result;
	}

	template <std::size_t index>
	static std::enable_if_t <index < sizeof... (T), disjoint_union>
	create (nth_type <index, T...>&& obj)
	{
		disjoint_union result (index);
		new (&result._storage) nth_type <index, T...> (std::move (obj));
		return result;
	}
	...
	template <std::size_t index>
	nth_type <index, T...>& get_unsafe ()
	{
		return *static_cast <nth_type <index, T...>*> (&_storage);
	}

	template <std::size_t index>
	const nth_type <index, T...>& get_unsafe () const
	{
		return *static_cast <const nth_type <index, T...>*> (&_storage);
	}

	template <std::size_t index>
	nth_type <index, T...>& get ()
	{
		if (_index != index)
	...
	template <std::size_t index>
	const nth_type <index, T...>& get () const
	{
		if (_index != index)
			throw disjoint_union_access_error ();
		return get_unsafe ();
	...
	elim (F... f) &
	{
		...
		R (*callers [sizeof... (T)]) (void*, void*) = {
			erase_unary_lvalue_function <F, T, R> ()...
		};
	...
	elim (F... f) const &
	{
		...
		R (*callers [sizeof... (T)]) (void*, const void*) = {
			erase_unary_const_lvalue_function <F, T, R> ()...
		};
		void* fobjs [sizeof... (T)] = {&f...};
	...
	elim (F... f) &&
	{
		...
		R (*callers [sizeof... (T)]) (void*, void*) = {
			erase_unary_rvalue_function <F, T, R> ()...
		};
		void* fobjs [sizeof... (T)] = {&f...};
	...
	apply (G g) &&
	{
		return std::move (*this).elim (replicate <G, T> (g)...);
	}
};
